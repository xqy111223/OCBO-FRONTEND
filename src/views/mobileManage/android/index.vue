<template>
  <div class="android-version">
    <div class="head mb15">
      <div class="head-item">
        <span>应用名称：</span>
        <el-select v-model="listQuery.applicationId" @focus="getApp" size="round" placeholder="请选择应用名称" clearable>
    ...
    <div class="main">
      <el-table :data="list" v-loading="loading" row-class-name="table-body-default" header-row-class-name="table-header-default">
        ...
        <el-table-column prop="upgradeInd" label="升级模式" width="100"></el-table-column>
        <el-table-column label="PHONE下载" width="100">
          <template slot-scope="scope">
            <a type="btn" @click="download(scope.row.phone)">下载</a>
          </template>
        </el-table-column>
        <el-table-column label="PAD下载" width="100">
          <template slot-scope="scope">
            <a type="btn" @click="download(scope.row.pad)">下载</a>
          </template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true" label="下载地址" width="500">
          <template slot-scope="scope">
            <div>
              <label>Phone：</label>{{scope.row.phone}}
            </div>
            <div>
              <label>Pad：</label>{{scope.row.pad}}
            </div>
          </template>
        </el-table-column>
        <el-table-column fixed="right" label="操作" width="120">
          <template slot-scope="scope">
            <a type="btn" @click="edit(scope.row.adminAppAndroidId)">编辑</a>
            <a type="btn" @click="del(scope.row.adminAppAndroidId)" color="danger">删除</a>
          </template>
        </el-table-column>
      </el-table>
      <!--      查看弹窗-->
...
export default {
  ...
  methods: {
    ...
    async getList (newVal) {
      ...
      try {
        const result = await getAndroidList(this.listQuery)
    ...
    async edit (id) {
      ...
      this.type = 'edit'
      try {
        const result = await androidDetail(id)
        this.info = result
      } catch (e) {
        this.$message({
          message: e.message,
    ...
    async confirm (form) {
      ...
      try {
        if (form.id) {
          await editAndroid(param)
        } else {
          await addAndroid(param)
        }
      } catch (e) {
        this.$message({
          message: e.message,
    ...
    async download (fileName) {
      try {
        await androidDownload(fileName)
      } catch (e) {
        this.$message({
          message: e.message,
          type: 'error'
