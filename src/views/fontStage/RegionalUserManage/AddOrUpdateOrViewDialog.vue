  ...
  <div class="addOrUpdateDialog">
    <el-dialog
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="100px"
               class="demo-ruleForm">
        <el-form-item v-if="dialogType!=='view'" label="用户名：" prop="loginName">
          <el-input v-model.trim="form.loginName" size="round" autocomplete="off" maxlength="30"></el-input>
        </el-form-item>
        <el-form-item v-if="dialogType=='view'" label="用户名：">
          <span>{{ userInfo.loginName }}</span>
        </el-form-item>
        <el-form-item v-if="dialogType!=='view'" label="密码：" prop="password">
          <el-input v-model.trim="form.password" size="round" autocomplete="off"></el-input>
          <el-button type="custom" @click="randomPassword" round>随机密码</el-button>
        </el-form-item>
        <el-form-item label="姓名：" prop="userName">
          <el-input v-if="dialogType!=='view'" v-model.trim="form.userName" size="round"
        ...
        <el-form-item label="区域：" prop="areaId">
          ...
          <span v-if="dialogType!='add'">{{ userInfo.areaPath }}</span>
        </el-form-item>
        <el-form-item v-if="dialogType=='view'" label="用户类型：">
          <span>{{getStatusCode(userInfo.isFamous,statusList)}}</span>
        </el-form-item>
        ...
        <el-form-item label="状态：" v-if="dialogType!=='add'">
          <el-radio-group v-if="dialogType==='add'" v-model="form.status">
...
export default {
  ...
  props: ['visible', 'dialogTitle', 'userInfo', 'dialogType', 'getStatus'],
  data () {
