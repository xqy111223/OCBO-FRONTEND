<template>
  <div class="schoolUserRecycleBin">
    <div class="head mb15">
      <div class="head-item">
        <span>区域：</span>
<!--        <select-region :area-id="listQuery.areaId" :value.sync="listQuery.areaId" :areaCode="areaCode" :filterable="true" clearble></select-region>-->
      ...
      <div class="head-item">
        <span>学段：</span>
        <el-input v-model.trim="listQuery.semesterId" size="round" placeholder="请输入用户名"></el-input>
      </div>
      <div class="head-item">
        <span>用户名：</span>
...
<script>
/*
* Description schoolUserRecycleBin
* Created By admin
* Created At 2019/9/18 14:08
*/
import { getSchoolRecycleDataList, viewUserInfo } from '@/api/fontStage/schoolUserRecycleBin'
...
export default {
  ...
  methods: {
    ...
    async getList (newVal) {
      ...
      try {
        const result = await getSchoolRecycleDataList(this.listQuery)
    ...
    async view (userId) {
      ...
      this.visible = true
      try {
        const result = await viewUserInfo(userId)
