    ...
    <el-dialog
      ...
      <el-form ref="form" :model="form" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="班主任：">
          <span>{{ getDetailedInfo(userInfo.gradeClassInfo,'headTeacher') }}</span>
        </el-form-item>
        <el-form-item label="任课信息：">
          <span>{{ getDetailedInfo(userInfo.headerSubjectInfo,'subjectInfo') }}</span>
        </el-form-item>
        <el-form-item label="是否名师：">
          <span>{{ userInfo.isFamous ? '是':'否' }}</span>
        </el-form-item>
        ...
        <el-form-item label="状态：">
          ...
          <span>{{ userInfo.status = 1 ? '开启' : '关闭' }}</span>
        </el-form-item>
      </el-form>
