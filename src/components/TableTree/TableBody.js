...
export default {
  ...
  methods: {
    ...
    handleEvent ($event, type, data, others) {
      const certainType = this.validateType(type, ['cell', 'row', 'icon'], 'handleEvent')
  ...
  render () {
    ...
    function getClassName (type, row, rowIndex, column, columnIndex) {
      ...
      if (certainType.row || certainType.cell) {
        ...
        if (certainType.row) {
          ...
          if (row._level > 1) {
            classList.push(`${this.prefixCls}__body-row__level`)
          }
    ...
    function renderCell (row, rowIndex, column, columnIndex) {
      ...
      if (this.table.treeType && this.table.firstProp === column.prop) {
        return <span
          class={`${this.prefixCls}--level-${row._level}-cell ${this.prefixCls}__firstProp`}
          style={{
        ...
        >
            {row._childrenLen > 0 &&
            <i class={`${this.prefixCls}--tree-icon el-icon-custom-${row._isFold ? 'fold' : 'extend'}`}></i>
            }
